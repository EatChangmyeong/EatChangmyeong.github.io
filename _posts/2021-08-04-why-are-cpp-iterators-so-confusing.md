...
이쯤 되면 천동설에서 주전원과 이심원 같은 걸 도입해서 천체의 운동을 간신히 설명하던 걸 지동설로 간단히 해결했던 게 생각나네요. 실제로 *오컴의 면도날*이라는 철학적 개념이 있는데, 거칠게 정리하자면 똑같은 현상을 설명할 수 있으면 더 간단한 설명이 낫다는 원리입니다. 프로그래밍 개념을 이해하는 데 이왕이면 더 간단한 멘탈 모델이 낫죠.
