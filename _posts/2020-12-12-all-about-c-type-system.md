...
오호! 3번 빼고 전부 헷갈리는 것들뿐이군요. 여기서 뿌린 떡밥은 글이 끝나기 전까지 전부 회수할 예정이니 걱정 말아주세요. 이 글을 읽으면서 C 타입 시스템은 왜 그렇게 복잡한지 조금이라도 이해할 수 있게 된다면 좋겠습니다.
...
* C 타입 시스템에서 제가 설명할 수 있을 정도로 이해한 모든 부분을 얕게 설명합니다. 세부사항은 검색해서 확인해 주세요. [cppreference.com](https://cppreference.com/) 자료를 권장드립니다.
	...
	* 구글링을 할 때는 밑줄 친 부분에 마우스를 올려서 나오는 단어를 참고해 주세요. 제가 맘대로 번역한 단어가 섞여있을 수 있으니 괜찮은/통용되는 번역어가 있다면 제보해 주세요.
...
`BaseType`과 `declarator`의 두 부분으로 나뉘는데, 간단히 말해 `BaseType`은 선언할 것의 기본 타입, `declarator`는 선언할 것의 이름을 나타냅니다. `declarator`는 0개 이상을 콤마로 구분해서 작성할 수 있는데, 그렇다고 진짜로 0개를 선언하면 의미가 없죠.

`declarator`는 선언할 식별자 이외에도 `*`나 `[]` 등을 포함합니다. 즉, `int *x;`라고 썼다면 `BaseType`은 `int`, `declarator`는 `*x`입니다. 이제 `int *x;`가 맞는 이유가 확실해졌습니다.
...
`declarator`는 다음 중 하나를 만족하는 것으로 정의됩니다. 실제로는 조금 더 복잡한데, 글을 쓰면서 하나하나 짚어볼 예정입니다.

* 식별자 (정의되는 변수, 함수, 타입 등의 이름)
* 기존의 `declarator`에...
	* `()`를 감싼 것 (연산자 우선순위를 나타냅니다.)
	* `*`를 앞에 붙인 것 ([포인터](#포인터))
	* `[]`를 뒤에 붙인 것 ([배열](#배열))
	* `(매개변수 0개 이상...)`를 뒤에 붙인 것 ([함수](#함수))
		* 함수의 매개변수는 각각 `declarator`가 1개인 선언문이나 타입 이름 꼴입니다.

이제부터는 여기에 나온 개념들을 하나하나 짚어보겠습니다.

# 타입 지정자

이 글에서는 타입을 크게 [기본 타입](#기본-타입), [파생 타입](#파생-타입), [복합 타입](#복합-타입)으로 나누며, 이중 파생 타입과 복합 타입은 다른 글에서와 조금 다른 의미로 사용합니다.

* [포인터](#포인터), [배열](#배열), [함수](#함수)는 파생 타입입니다.
* [구조체](#구조체), [공용체](#공용체), [열거형](#열거형)은 복합 타입입니다.
	* 열거형은 구조체와 공용체와는 성질이 다르지만, 선언 문법이 비슷하고 여러 개의 원소를 같이 선언하기 때문에 복합 타입으로 분류했습니다.
...
* 아래에서 설명하겠지만, 여러 단어 타입의 경우 단어의 순서를 자유롭게 바꿀 수 있습니다.
...
`n`이 컴파일 타임 상수가 아닐 경우 이를 "가변 크기 배열"(variable-length array, VLA)이라고 하며, VLA의 타입 혹은 여기서 파생되는 타입을 가변 타입이라고 합니다. 일반 배열과 달리 컴파일 타임에는 크기가 결정되지 않고, 선언문 부분까지 실행될 때에서야 완전히 결정됩니다.
...
* 매개변수가 없을 경우(`int x();`)에는 명시되지는 않지만 고정된 수의 인자를 받을 수 있습니다(`x(123)`). 매개변수가 `void`인 경우(`int x(void);`)에는 명시적으로 인자를 넣을 수 없다는 의미가 됩니다.
...
`...` 부분에 `float`를 전달하면 `double`로, 비트 필드를 포함해 부호 유무와 무관하게 `int`보다 작은 정수 타입을 전달하면 `int` 혹은 `unsigned int`로 변환됩니다. `printf` 함수에서 `float`와 `double`을 구분 없이 `%f`로 출력하는 것도 이런 이유입니다. `float _Complex`와 `float _Imaginary`는 변환되지 않습니다.
...
1. 지금까지 읽은 선언이 괄호로 감싸져 있을 경우 2번으로 돌아간 뒤 여닫는 괄호부터 읽는다.
...
>
> * (/4) *함수 지시자*란 함수 타입을 가지는 표현식을 말한다. **sizeof** 연산자나 단항 & 연산자의 피연산자일 때를 제외하면 "*type*을 반환하는 함수" 타입을 가지는 함수 지시자는 "*type*을 반환하는 함수의 포인터" 타입을 가지는 표현식으로 변환된다.
...
`typedef`도 일반적인 선언과 같이 블록 범위를 가질 수 있습니다. 안쪽에 있는 `typedef`가 바깥쪽의 `typedef`를 가립니다.
...
재미있는 사실! 의외로 `typedef`는 이론상 기억 영역 분류 지정자입니다. 물론 `typedef`는 기억 영역 분류와 관련해 어떤 동작도 하지 않습니다. 위에서 "`BaseType`은 기억 영역 분류 지정자를 포함하며, 순서는 상관 없다"고 했던 걸 생각하면 의외로 이렇게도 할 수 있습니다.
...
복합 타입 선언은 크게 이름 부분과 정의 부분으로 나뉘고, 포함하느냐 마느냐에 따라 서로 다른 의미를 갖습니다. 아래 세 종류의 구문 모두 `BaseType`으로 쓸 수 있기 때문에 변수 선언까지 할 수 있습니다.
...
변수를 메모리에 할당하려면 타입의 크기를 알아야 하는데, 충분한 정보가 없어 크기를 결정할 수 없는 경우도 있습니다. 이런 상태의 타입을 불완전 타입이라고 합니다. 불완전 타입을 가지는 변수는 선언할 수 없습니다.
...
* 정의를 마치지 않은 구조체 및 공용체
	* 정의를 마치는 순간(즉, 중괄호 `{ ... }`가 닫히는 순간) 완성됩니다. 이전 위치에 있는 구조체/공용체 참조는 완성되지 않은 상태로 남습니다.
...
* 불완전 타입의 배열은 만들 수 없습니다.
	* 다차원 배열에서 가장 왼쪽의 크기만 생략할 수 있는 것(`int x[][1][2][3];`)도 이런 이유입니다.
...
원래는 글에 포함하지 않으려고 했는데, 이곳저곳에서 언급하니까 한 번쯤 짚고 넘어가야 할 것 같네요.

C에서 모든 식별자는 범위를 가지며, 그 범위 안에 있는 코드만 그 식별자를 참조할 수 있습니다. C에는 크게 두(+2) 가지의 범위가 있습니다. `typedef`와 복합 타입 선언을 포함해 모든 선언은 아래의 범위 규칙을 따릅니다.

## 블록 범위

블록(`{ ... }`, 매개변수와 함수 본문을 포함합니다) 안에 선언한 식별자는 선언이 끝나는 곳부터(초기화가 될 경우, 초기화 표현식 바로 앞부터) 그 블록이 끝날 때까지 참조할 수 있습니다. 안쪽 블록과 바깥쪽 블록에서 같은 식별자를 선언한 경우 안쪽이 바깥쪽을 가립니다.
...
```c
for(int i = 0 /* scope of i begins here */; i < 10; i++) {
	foo(i); // OK
...
그렇지 않은 식별자는 선언이 끝나는 곳부터 파일이 끝날 때까지 참조할 수 있습니다. 파일 범위로 선언된 변수는 흔히 "전역 변수"라고도 합니다.

## 함수 범위

함수 안의 레이블은 그 함수의 본문 전체(레이블보다 앞인 위치를 포함합니다)에서 참조할 수 있습니다.
...
기억 영역 분류는 크게 **기억 기간**과 **연결성**으로 나뉩니다. 위에서 언급한 다섯 종류의 지정자를 알아보기 전에 먼저 살펴봅시다.
...
**연결성**은 식별자를 다른 범위에서 참조할 수 있는지의 여부를 결정합니다. 한 파일 내에서 한 식별자가 내부 연결성과 외부 연결성을 동시에 띨 수 없습니다.

* **연결성 없음**: 해당 식별자를 선언한 범위에서만 참조할 수 있습니다.
	* 연결성 없이 선언되었을 경우 식별자가 같아도 모두 다른 것을 가리킵니다.
* **내부 연결**: 해당 식별자가 있는 파일 전체에서 참조할 수 있습니다.
	* 정확히 말하자면, 특정한 파일에서 내부 연결성으로 선언되었을 때, 식별자가 같다면 같은 것을 가리킵니다.
* **외부 연결**: 다른 파일을 포함해 프로그램 전체에서 참조할 수 있습니다. 링크 단계에서 외부 연결성을 지닌 식별자 참조끼리 연결됩니다.
	* 정확히 말하자면, 컴파일에 사용된 모든 파일에서 외부 연결성으로 선언되었을 때, 식별자가 같다면 같은 것을 가리킵니다.

블록 범위에서 외부 연결성으로 선언했을 때는 조금 복잡한 규칙을 추가로 따르는데,

* 기존에 선언했던 같은 식별자 중 "보이는"(참조되는) 것이 `static`이나 `extern`이라면, 그 식별자의 연결성을 상속받습니다.
...
**기억 영역 분류 지정자**를 명시하면 변수의 생명주기가 어떻게 되는지, 다른 곳에서 참조할 수 있는지를 나타낼 수 있습니다.

* `auto`: 자동 기억 기간을 가지고, 연결성은 없습니다.
	* 매개변수를 제외한 블록 범위 변수에만 사용할 수 있습니다.
	* 매개변수와 블록 범위 변수에 기본값으로 적용됩니다.
* `register`: `auto`와 같지만, 컴파일러가 이 변수의 값을 CPU 레지스터에 저장하도록 최적화할 수 있습니다. 실제로 최적화가 이루어지는지와 무관하게 `&` 연산자를 사용할 수 없습니다.
	* 매개변수를 포함해 블록 범위 변수에만 사용할 수 있습니다.
* `static`: 정적 기억 기간을 가지고, 내부 연결성을 가지거나, 블록 범위일 경우 연결성을 가지지 않습니다.
	* 파일 범위의 함수와 매개변수를 제외한 모든 변수에 사용할 수 있습니다.
* `extern`: 정적 기억 기간과 외부 연결성을 가집니다.
	* 모든 함수와 매개변수를 제외한 모든 변수에 사용할 수 있습니다.
	* 모든 함수와 파일 범위 변수에 기본값으로 적용됩니다.
* `_Thread_local` *(C11~)*: 스레드 기억 기간을 가집니다.
	* 매개변수를 제외한 변수에만 사용할 수 있습니다. 블록 범위 변수일 경우 아래와 같은 형태로 써야 합니다.
		* `static _Thread_local`: 내부 연결성을 가집니다.
		* `extern _Thread_local`: 외부 연결성을 가집니다.
	* `#include <threads.h>`를 하면 `thread_local`로 쓸 수 있습니다.

위의 기억 영역 분류는 선언의 종류에 따라 다음과 같이 적용됩니다.

* 파일 범위 함수는 기본값이 `extern`이며, 이외에 `static`으로 선언할 수 있습니다.
* 블록 범위 함수 선언은 기본값이 `extern`이며, 다른 지정자로 선언할 수 없습니다.
* 파일 범위 변수는 기본값이 `extern`이며, 이외에 `static`이나 `_Thread_local`로 선언할 수 있습니다.
* 블록 범위 변수는 기본값이 `auto`이며, `register`, `static`, `extern`, `_Thread_local`로 모두 선언할 수 있습니다.
* 매개변수는 기본값이 `auto`이며, `register`로 선언할 수 있습니다. 명시적으로 `auto`로 선언할 수는 없습니다.
...
포인터에 붙는 경우 포인터의 바로 뒤에 붙습니다(`* const` 혹은 `*const`). Rust를 써본 적이 있다면, 참조에 `mut` 키워드가 붙는 규칙과 같다(`&mut`)고 이해하면 외우기 쉽습니다. 개인적으로 C에서도 의도를 명확히 전달하기 위해 포인터와 한정자 사이를 띄우지 않는 것이 좋다고 생각합니다(`*const`처럼).
...
1. `const int *x;`: `int`가 `const` 한정되었습니다. 포인터(`*`)인 `x`는 `const`가 아니지만, `const int`인 `*x`는 `const`입니다.
2. `int const *x;`: `int`가 `const` 한정되었습니다. 1번과 같습니다.
3. `int *const x;`: `*`가 `const` 한정되었습니다. 포인터(`*const`)인 `x`는 `const`이지만, `int`인 `*x`는 `const`가 아닙니다.
4. `int const *const x;`: `int`와 `*`가 모두 `const` 한정되었습니다. `x`와 `*x`가 모두 `const`입니다.
...
// ↑ 컴파일러에서 출력한 타입 이름 'int const[8]'은
// "a (const-qualified `[8]`) of `int`"를 의미합니다.
// 실제 코드에 위와 같이 쓰면
// "a `[8]` of const-qualified `int`로 인식됩니다.
...
// ↑ 사실 경고 메시지가 왜 저렇게 나오는지 모르겠습니다. 제보 부탁드립니다
```
...
printf("%d", x); // 뭐가 출력될지 모릅니다.
```

`const` 한정된 변수는 값이 변하지 않기 때문에, 읽기 전용 영역에 할당하거나 상황에 따라 아예 하드코딩이 되도록 최적화될 수 있습니다.

## `volatile`: "최적화되지 않음"

`volatile` 한정자가 붙은 타입의 읽기/쓰기 연산은 부작용[^fn-side-effect]으로 취급되며, 최적화 없이 있는 그대로 실행됩니다. 주로 값이 불안정하거나, 입출력 신호가 메모리 매핑이 되어 있거나, 벤치마크 등 최적화를 해서는 안 되는 상황에 사용합니다. 이건 코드만으로는 보여줄 수 없으니 컴파일러에서 출력하는 어셈블리를 직접 확인해 봅시다.
...
`restrict`는 함수 포인터가 아닌 포인터만을 한정할 수 있습니다.
...
컴파일러는 `restrict` 포인터가 다른 모든 포인터와 다른 객체를 참조한다고 가정하고 최적화할 수 있습니다. 예를 들어, 아래 코드의 `bar`가 `restrict` 포인터를 받지 않는다고 가정하면 `x == y`인 경우를 고려해 조금 더 긴 어셈블리가 출력되지만, `restrict`로 인해 `x != y`가 보장되므로 어셈블리 출력이 줄어듭니다.
...
bar(&a, &b); // OK, x and y each points to different object
...
*[밑줄 친 부분에 마우스를 올려서 나오는 단어]: 이렇게 표시됩니다.
*[가변 타입]: variably-modified type
...
*[타입 이름]: type name
*[조건부 체이닝]: optional chaining
*[좌측값]: lvalue
*[파생 타입]: derived type
*[한정자]: qualifier
*[함수 지시자]: function designator
*[함수 지정자]: function specifier
...
[^fn-function-specifier]: 함수에 부가적인 성질을 부여합니다. `inline`은 컴파일러가 인라이닝을 해도 되는 함수, `_Noreturn`은 `exit`나 `longjmp` 등의 이유로 인해 절대 반환하거나 끝까지 실행되지 않는 함수를 의미합니다. 이 글에서 자세히 다루지는 않겠습니다.
[^fn-lvalue]: 메모리상에 "정체"가 있는 값을 말합니다. 다른 이유로 제한되는 경우가 아니면 대입하거나 `&` 연산자로 주소를 얻을 수 있습니다.
