...
**현재 파이어폭스에서 인터랙티브 애니메이션이 정상적으로 재생되지 않는 문제가 있습니다.** 정식으로 공개하기 전까지 수정해서 올리겠습니다.
...
위에서 만들어지는 형태와 같은 *LIS 트리*(급조한 이름입니다)를 생각해 봅시다. 루트는 가상의 "0번째" 원소, 즉 $$-\infty$$고 수열의 각 원소가 *자기 자신으로 끝나는 LIS를 이루기 좋은 곳*에 자식으로 붙습니다. 모든 원소는 수열에 등장하는 순서대로 처리되므로 순서 문제로 LIS를 이루지 않는 경우는 없습니다.
