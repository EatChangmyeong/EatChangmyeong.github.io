...
* 아직 확정되지 않은 C23에서의 변경 사항을 글에 미리 반영했습니다. 해당 내용은 시간이 지남에 따라 바뀔 수 있습니다.
...
C 표준이 있기 전에는 [*The C Programming Language*](https://en.wikipedia.org/wiki/The_C_Programming_Language)(`hello, world`를 대중화시킨 그 책이 맞습니다)가 사실상의 표준 역할을 했습니다. 이 시절의 C "표준"을 저자 이름의 앞글자를 따서 K&R C라고 합니다. 이후 1989년에 최초로 C89라는 표준이 생겼고, 이후 C95, C99, C11, C17까지 4번의 개정을 거쳤습니다. C17은 기능 추가 없이 결함만 수정했습니다.
...
* C17: `-std=c17`
* C2x: `-std=c2x` (다음 표준 개정판인 C23을 미리 써볼 수 있습니다. 컴파일러 버전에 따라 동작이 바뀌거나 불안정할 수 있습니다)
...
* 이진 부동소숫점: 아래의 키워드 2종류에서 하나씩을 조합해서 쓸 수 있습니다.
	...
	* 복소수 여부 *(C99~)*: *`(없음)`* (실수), `_Imaginary` (허수), `_Complex` (복소수)
		...
		* 모든 컴파일러에서 복소수를 지원하는 것은 아니며, 지원할 경우 매크로 `__STDC_IEC_559_COMPLEX__`*(C23부터 deprecated)*가, 그렇지 않을 경우 `__STDC_NO_COMPLEX__`*(C11~)*가 1로 정의되어 있어야 하지만, 왠지 모르게 둘 다 정의되어 있지 않은 컴파일러가 있는 것 같습니다.
...
* C 스타일로 작성할 수 있는 파생 타입은 아래와 같이 5가지입니다.
	...
	* 배열 `BaseType x[n];`
	* 함수 `BaseType x(args...);`
		* `BaseType x();`라고 선언했는데 클래스 `BaseType`의 기본 생성자(`BaseType::BaseType()`)가 호출되지 않는 것도 매개변수가 없고 `BaseType`을 반환하는 함수로 해석되기 때문입니다.
	* 좌측값 참조 `BaseType &x;`
...
* (C++ 한정) 참조의 포인터, 참조의 배열, 참조의 참조는 만들 수 없습니다.
* 글의 나중에 나오는 내용이지만 [선언에서 식별자를 지우면 타입 이름이 됩니다](#타입-이름-type-name). `int`의 포인터의 벡터를 만들고 싶다면 `std::vector<int *>`처럼 하면 됩니다.
...
함수를 한정하는 문법도 없으며, `typedef`를 이용해 함수를 *`const`/`volatile`* 한정하려고 하면 정의되지 않은 동작이 되고, *`restrict`* 한정하려고 하면 오류가 발생합니다.
...
[^fn-devcpp]: 이 트윗의 내용을 부연하자면, Dev-C++는 컴파일에 [TDM-GCC](https://jmeubank.github.io/tdm-gcc/)를 씁니다. 이전 버전 Dev-C++는 딸려오는 TDM-GCC도 이전 버전인데(4.x.x), 이때는 [플래그를 지정하지 않으면 C89 표준에 컴파일러 확장을 허용하여(`-std=gnu90`) 컴파일을 했었습니다](https://gcc.gnu.org/onlinedocs/gcc-4.9.4/gcc/Standards.html#Standards).
